...
function removeThe(name) {
    let n = name;
    while (n.includes('the ')) n = n.replace('the ', '').trim();
    return n;
}
...
class Schema {
    ...
    setPropertiesData() {
        // reset lazy caches so they rebuild from the new raw data
        this.paints = undefined;
        this._craftableWeaponsCache = null;
        this._paintableDefindexesCache = null;
        this._strangePartsCache = null;
        ...
        this._buildItemIndexes();
    }

    _buildItemIndexes() {
        // o(1) maps built once on schema load, used by getItemByDefindex/Name
        const items = this.raw.schema.items;
        const byDefindex = new Map();
        const byName = new Map();
        const byNameNoThe = new Map();

        for (let i = 0; i < items.length; i++) {
            const item = items[i];
            byDefindex.set(item.defindex, item);
            ...
            const normalized = normalizeNameKey(item.item_name);
            if (!byName.has(normalized)) byName.set(normalized, item);

            const noThe = removeThe(lower);
            if (!byNameNoThe.has(noThe)) byNameNoThe.set(noThe, item);

            const normalizedNoThe = removeThe(normalized);
            if (!byNameNoThe.has(normalizedNoThe)) byNameNoThe.set(normalizedNoThe, item);
        }

        this._itemsByDefindex = byDefindex;
        this._itemsByName = byName;
        this._itemsByNameNoThe = byNameNoThe;
    }

    getItemByItemNameWithThe(name) {
        const n = removeThe(normalizeNameKey(name));
        return this._itemsByNameNoThe.get(n) ?? null;
    }
    ...
    getItemByDefindex(defindex) {
        return this._itemsByDefindex.get(defindex) ?? null;
    }
    ...
    getItemByItemName(name) {
        return this._itemsByName.get(normalizeNameKey(name)) ?? null;
    }
    ...
    getPaintNameByDecimal(decimal) {
        return this._paintDecimalToName.get(decimal) ?? null;
    }
    ...
    getPaintDecimalByName(name) {
        return this._paintNameToDecimal.get(name.toLowerCase()) ?? null;
    }
    ...
    getPaints() {
        ...
        const toObject = {};
        const decimalToName = new Map();
        const nameToDecimal = new Map();
        const paintCansCount = paintCans.length;

        for (let i = 0; i < paintCansCount; i++) {
            const can = paintCans[i];
            if (can.attributes === undefined) continue;
            const decimal = can.attributes[0].value;
            toObject[can.item_name] = decimal;
            decimalToName.set(decimal, can.item_name);
            nameToDecimal.set(can.item_name.toLowerCase(), decimal);
        }

        toObject['Legacy Paint'] = 5801378;
        decimalToName.set(5801378, 'Legacy Paint');
        nameToDecimal.set('legacy paint', 5801378);

        this._paintDecimalToName = decimalToName;
        this._paintNameToDecimal = nameToDecimal;
    ...
    getPaintableItemDefindexes() {
        if (this._paintableDefindexesCache) return this._paintableDefindexesCache;
        this._paintableDefindexesCache = this.raw.schema.items
            .filter(item => item.capabilities && item.capabilities.paintable === true)
            .map(item => item.defindex);
        return this._paintableDefindexesCache;
    }
     ...
     */
    getStrangeParts() {
        if (this._strangePartsCache) return this._strangePartsCache;
        ...
        this._strangePartsCache = toObject;
        return this._strangePartsCache;
    }
    ...
    getCraftableWeaponsSchema() {
        if (this._craftableWeaponsCache) return this._craftableWeaponsCache;
        ...
        this._craftableWeaponsCache = this.raw.schema.items.filter(
            item =>
                !weaponsToExclude.includes(item.defindex) && item.item_quality === 6 && item.craft_class === 'weapon'
        );
        return this._craftableWeaponsCache;
    }
